...
class ListFleetsResponse(BaseModel):
    ...
    """ # noqa: E501
    fleets: List[Fleet] = Field(description="Array of fleet objects")
    next_token: Optional[StrictStr] = Field(default=None, description="Pagination token to retrieve the next page of results. Null when pagination not used or no more pages available.", alias="nextToken")
    limit: Optional[Annotated[int, Field(le=100, strict=True, ge=1)]] = Field(default=None, description="Number of items requested per page. Null when pagination not used.")
    __properties: ClassVar[List[str]] = ["fleets", "nextToken", "limit"]
